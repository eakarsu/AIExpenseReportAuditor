...
export default function AITaxDeductions() {
  ...
  const analyze = async (id) => {
    ...
    try { const data = await api.taxDeductions(reportId); setResult(data) }
  ...
  const samples = [
    ...
    { label: 'Travel-Heavy Report', action: () => { const r = reports.find(r2 => r2.title?.toLowerCase().includes('travel') || r2.title?.toLowerCase().includes('trip')); if (r) { setSelectedId(String(r.id)); analyze(String(r.id)) } else if (reports[0]) { setSelectedId(String(reports[0].id)); analyze(String(reports[0].id)) } } },
    { label: 'Client Entertainment', action: () => { const r = reports.find(r2 => r2.title?.toLowerCase().includes('client') || r2.title?.toLowerCase().includes('dinner')); if (r) { setSelectedId(String(r.id)); analyze(String(r.id)) } else if (reports[1]) { setSelectedId(String(reports[1].id)); analyze(String(reports[1].id)) } } },
    { label: 'Approved Report', action: () => { const r = reports.find(r2 => r2.status === 'approved'); if (r) { setSelectedId(String(r.id)); analyze(String(r.id)) } } },
  ]

  return (
    <div>
      <div className="page-header"><div><h1>{'\u{1F4B3}'} AI Tax Deduction Analyzer</h1><div className="breadcrumb">Identify tax-deductible expenses and optimize deductions</div></div></div>
      <div className="detail-card"><div className="detail-body">
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'16px',marginBottom:'16px'}}>
          ...
          <div className="form-group" style={{marginBottom:0}}><label>Categories ({categories.length})</label>
            <select disabled><option>AI checks {categories.length} categories</option>
              {categories.map(c => <option key={c.id}>{c.name} ({c.code})</option>)}
            </select>
          </div>
        </div>
        <button className="btn btn-primary" onClick={() => analyze()} disabled={!selectedId||loading}>{loading ? 'Analyzing...' : 'Analyze Tax Deductions'}</button>
      </div></div>
